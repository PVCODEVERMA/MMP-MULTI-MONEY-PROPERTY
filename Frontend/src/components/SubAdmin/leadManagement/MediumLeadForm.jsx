...
import { useMediumLeads } from "../../../context/MediumLeadContext";
import toast from "react-hot-toast";

const MediumLeadForm = () => {
  const {
    mediumLeads,
    loading,
    createMediumLead,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editLeadId) {
        await updateMediumLead(editLeadId, formData);
      } else {
        await createMediumLead(formData);
      }
      setFormData({ name: "", phone: "", email: "", propertyType: "", status: "new" });
  ...
  return (
    <div className="max-w-7xl mt-10 mb-10 p-4">
      <div className="flex justify-between items-center mb-5">
        <div>
          <h1 className="text-2xl font-bold text-gray-800">Medium Intent Leads</h1>
          <p className="text-gray-600 mt-2">
            Manage and track all medium intent leads
          </p>
        </div>
        <button
          onClick={() => {
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-200 overflow-hidden">
        <div className="bg-gradient-to-r from-[#154056] to-[#1e5a7a] p-6 text-white">
          <h2 className="text-xl font-bold">All Medium Leads</h2>
          <p className="text-blue-100 mt-1">Total {mediumLeads.length} leads</p>
        </div>
        ...
        <div className="overflow-x-auto">
          {loading ? (
            ...
            <p className="text-center py-6">No leads found. Add your first medium intent lead!</p>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-transparent flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 overflow-auto">
            <h2 className="text-2xl font-bold mb-4 text-center">
              {editLeadId ? "Edit Medium Lead" : "Add Medium Lead"}
            </h2>
