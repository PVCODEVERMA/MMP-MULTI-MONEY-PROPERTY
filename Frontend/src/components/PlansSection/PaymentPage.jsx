...
export default function PaymentPage() {
  ...
  // store add-ons in state (so we can remove them)
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    toast.success(
      ` Payment request for ${plan.name} (₹${totals.grandTotal.toLocaleString()}) submitted successfully!`
    );
    setPhone("");
    e.target.reset();
  };
  ...
  return (
    <div className="min-h-screen bg-[#f7f7f7] py-20 px-6">
      <div className="max-w-6xl mx-auto grid md:grid-cols-2 gap-8">
          ...
          {selectedAddOns.length > 0 && (
            <>
              ...
              <div className="mt-4 text-sm text-gray-800">
                <p>
                  Add-ons Total:{" "}
                  <strong>₹{totals.addOnTotal.toLocaleString()}</strong>
                </p>
              </div>
            </>
          )}
        ...
        {/* RIGHT: Payment Form */}
        <div className="bg-white shadow-lg rounded-2xl p-6">
          <h2 className="text-2xl font-bold text-[#164058] mb-4">
            Enter Your Details
          </h2>
          <form className="space-y-4" onSubmit={handleSubmit}>
            <input
              ...
              required
              className="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-[#FF9C00] outline-none"
            />
            <input
              type="email"
            ...
            <PhoneInput
              ...
              onChange={setPhone}
              className="w-full"
              inputClassName="w-full border rounded-lg px-3 py-2 focus:ring-2 focus:ring-[#FF9C00] outline-none"
            />
            ...
            <button
              type="submit"
              className="w-full py-3 rounded-full bg-[#FF9C00] text-white font-semibold hover:bg-[#e68800] transition-transform hover:scale-105 cursor-pointer"
            >
